...
    // Ensure we create the date in UTC to avoid timezone issues
    return new Date(Date.UTC(parseInt(year), new Date(`${month} 1, 2000`).getMonth(), parseInt(day)));
}
...
function convertLogseqToLexical(block: LogseqBlock, indent: number = 0, isJournalEntry: boolean = false): LexicalNode[] {
    let nodes: LexicalNode[] = [];
    
    // Only create h1 for page-name if it's not a journal entry
...
    const regularPages: LogseqBlock[] = [];

    // First, separate journal entries from regular pages
    if (Array.isArray(logseqData.blocks)) {
        logseqData.blocks
            .filter((block) => block['page-name'])
            .forEach((block) => {
                const pageName = block['page-name'] || '';
                // Strictly validate date format
                try {
...
        const lexicalNodes = convertLogseqToLexical(block);
        
        // Create the full Lexical content structure
...
            combinedNodes.push(...entryNodes);

            // Add extra newline between entries (except for the last one)
