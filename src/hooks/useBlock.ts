...
export function useBlock() {
    ...
    const modifyBlock = useCallback(async (id: string, updates: Partial<Block>) => {
        try {
    ...
    const getRecommendedBlocks = useCallback(async (blockId: string) => {
        try {
            ...
            dispatch({ type: 'SET_ERROR', error: error instanceof Error ? error.message : 'Failed to get recommended blocks' });
        } finally {
            dispatch({ type: 'FINISH_LOADING' });
        }
    }, [dispatch]);
